...
def create_sblex_server(*, title: str, settings: Settings) -> FastAPI:
    ...
    telemetry.init_otel_tracing(settings.otel, fallback_name=make_fallback_name(title))
...
def make_fallback_name(name: str) -> str:
    name = name.lower()
    return name.replace(" ", "-")
